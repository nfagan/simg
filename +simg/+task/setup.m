...
if ( nargin < 1 )
  conf = simg.config.load();
else
  simg.util.assertions.assert__is_config( conf );
end

conf = simg.config.reconcile( conf );
...
program.Destruct = @simg.task.shutdown;
...
task.Loop = @(t) simg.task.loop(t, program);
...
states('new_trial') = simg.task.states.new_trial( program, conf );
states('present_image') = simg.task.states.present_image( program, conf );
states('image_success') = simg.task.states.image_success( program, conf );
states('image_error') = simg.task.states.image_error( program, conf );
...
% the config file `conf`. `conf` is created in `simg.config.create`, 
